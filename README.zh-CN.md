...
| **导出主题 JSON** | 一键把当前主题（名称、配色、颜色格式）存成 JSON 文件；「导入」面板能把它读回来，方便备份或把主题交给别人 |
...
### `npm run verify` —— 294 条断言，不用打开浏览器
...
**211 条断言，分 23 个套件**：首屏渲染、实时预览的响应、输错之后能不能恢复、预设、随机配色、
